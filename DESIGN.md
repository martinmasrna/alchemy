...
Icons: the emoji are placeholders and the replacement is decided. Things that reflect light are cut paper: two flat planes with a cast shadow between them and no tile behind. Things that give off light (stars, nebulae, lava, energy) are drawn as light, with real glow, because paper cannot hold heat or majesty and the first review said so for every one of them. A piece of the world is drawn only when the thing is itself in relation to something else: Gravity's sheet, the Moon's planet, the Rock's pebbles. A thing is the colour a person already expects it to be, and the test is Earth. Chemical elements are periodic-table tiles with the symbol lit in the colour that gas really glows. The decision and its rules are in `design/shortlist/habitat-r/port-spec.md`; the world 1 set and what the drawing taught are in `design/shortlist/habitat-r/world1/`. Nothing is in the game yet.
...
## Where we are
World 1 (Big Bang to Earth, 27 discoveries) is live at https://martinmasrna.github.io/alchemy/ with the converged screen and the credit economy. Martin has played it twice (logs in `playlogs/`); the planet half has never been played blind, so the next real data point is a friend's log. All 31 of world 1's icons are drawn and 25 have passed review; Matter, Comet, Lava and Ocean are redrawn and back in the queue. Planet stays as it is unless Martin says otherwise, and Lightyear may be cut from the table. Once they pass, the next design job is putting them into the game. Next content: world 2 (Sun, Water, Rock, Air to Life).
